...
{
    public Task<Order?> GetActiveOrderByTableId(int tableId, bool trackChanges = false)
        => Task.FromResult(Data.FirstOrDefault(o => o.TableId == tableId));

    public Task CreateMultiplierOrderItems(int tableId, OrderItem[] items) => Task.CompletedTask;
}
