    ...
    return (
        <div className="space-y-5">
            ...
            {selectedTable && (
                <>
                    <div
                        className="fixed inset-0 z-60 bg-black/55 backdrop-blur-sm animate-in fade-in duration-200"
                        onClick={closePanel}
                    />
                    <div className="fixed top-0 right-0 bottom-0 z-70 w-full md:w-190 shadow-2xl animate-in slide-in-from-right duration-300 bg-background flex flex-col">
                        {/* Header */}
                                    ...
                                    <div className="flex-1 overflow-y-auto px-4">
                                        {filteredProducts.length === 0 ? (
                                            ...
                                            filteredProducts.map(product => {
                                                const cartItem = newItems.find(i => i.productId === product.id);
                                                                ...
                                                                <div className="mt-1.5 ml-15">
                                                                    {cartNoteEditorFor === product.id ? (
                                                                        <input
                                                                            autoFocus
                                                                            value={cartItem?.note ?? ''}
                                                                            onChange={e => updateNewItemNote(product.id, e.target.value)}
            ...
            {paymentOrder && (
                <>
                    <div
                        className="fixed inset-0 z-80 bg-black/60 backdrop-blur-[3px] animate-in fade-in duration-200"
                        onClick={() => setPaymentOrder(null)}
                    />
                    <div className="fixed top-0 right-0 bottom-0 z-80 w-full md:w-190 bg-[#FFFDF8] dark:bg-[#1C1712] border-l border-border shadow-[0_24px_64px_rgba(0,0,0,0.35)] animate-in slide-in-from-right duration-300">
                        <PaymentPanel
                            order={paymentOrder}
